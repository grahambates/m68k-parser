import { parseFile } from "../line-parser";

describe("parseFile", () => {
  describe("basic functionality", () => {
    it("should parse empty file", () => {
      const result = parseFile("");
      expect(result.totalLines).toBe(1); // Empty string splits to one empty line
    ...
    it("should parse single line", () => {
      ...
      expect(result.lines).toHaveLength(1);
      expect(result.lines[0].lineNumber).toBe(1);
      expect(result.lines[0].text).toBe("  move.w d0,d1");
      expect(result.lines[0].parsed.mnemonic?.type).toBe("instruction");
      expect(result.lines[0].parsed.operands).toHaveLength(2);
    });
  ...
  bra.s label`;
      ...
      expect(result.lines[1].text).toBe("  add.l d2,d3");

      expect(result.lines[2].lineNumber).toBe(3);
      expect(result.lines[2].text).toBe("  bra.s label");
    });
  });
  ...
  describe("line ending normalization", () => {
    ...
    it("should handle mixed line endings", () => {
      ...
      expect(result.lines[1].text).toBe("line2");
      expect(result.lines[2].text).toBe("line3");
      expect(result.lines[3].text).toBe("line4");
    });
  });
  ...
  describe("error handling and aggregation", () => {
    it("should collect errors from single line", () => {
      ...
      expect(result.errors[0].error.code).toBe("INVALID_BASE_REGISTER");
    });
  ...
  sub.w 30(d4),d5`;
      const result = parseFile(source);
      expect(result.totalLines).toBe(3);
      expect(result.errorCount).toBe(3); // All have data registers as base
      expect(result.errors).toHaveLength(3);

      expect(result.errors[0].lineNumber).toBe(1);
      expect(result.errors[0].error.code).toBe("INVALID_BASE_REGISTER");

      expect(result.errors[1].lineNumber).toBe(2);
      expect(result.errors[1].error.code).toBe("INVALID_BASE_REGISTER");

      expect(result.errors[2].lineNumber).toBe(3);
      expect(result.errors[2].error.code).toBe("INVALID_BASE_REGISTER");
    });
    ...
    it("should handle multiple errors on same line", () => {
      ...
      const result = parseFile(source);
      expect(result.totalLines).toBe(1);
      expect(result.errorCount).toBe(1); // d0 as base register
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].lineNumber).toBe(1);
      expect(result.errors[0].error.code).toBe("INVALID_BASE_REGISTER");
    });
  ...
  or.w 20(d5),d6`;
      const result = parseFile(source);
      expect(result.totalLines).toBe(4);
      expect(result.errorCount).toBe(2); // d2 and d5 as base registers
...
data:
  dc.w 1,2,3`;
      ...
      // Check labels
      expect(result.lines[0].parsed.label?.label).toBe("start");
      expect(result.lines[2].parsed.label?.label).toBe("loop");
      expect(result.lines[5].parsed.label?.label).toBe("data");

      // Check instructions
      expect(result.lines[1].parsed.mnemonic?.type).toBe("instruction");
      expect(result.lines[3].parsed.mnemonic?.type).toBe("instruction");

      // Check directive
      expect(result.lines[6].parsed.mnemonic?.type).toBe("directive");
    });
...
* Another comment style
  add.l d2,d3`;
      ...
      // Line 1: just comment
      expect(result.lines[0].parsed.comment?.content).toContain("This is a comment");

      // Line 2: instruction with inline comment
      expect(result.lines[1].parsed.mnemonic?.type).toBe("instruction");
      expect(result.lines[1].parsed.comment?.content).toContain("inline comment");

      // Line 3: just comment
      expect(result.lines[2].parsed.comment?.content).toContain("Another comment style");
  ...
  sub.w \\<param>,d1`;
      ...
      expect(result.lines[1].parsed.operands?.[0]?.type).toBe("macro-parameter");
      expect(result.lines[1].parsed.operands?.[1]?.type).toBe("macro-parameter");
    });
...
`;
      ...
      // Non-empty lines
      expect(result.lines[0].parsed.mnemonic?.type).toBe("instruction");
      expect(result.lines[2].parsed.mnemonic?.type).toBe("instruction");

      // Empty lines
      expect(result.lines[1].text).toBe("");
      expect(result.lines[3].text).toBe("");
      expect(result.lines[4].text).toBe("");
    });
...
multiply_by_10:
  ...
  rts`;
      const result = parseFile(source);
      expect(result.totalLines).toBe(7); // 7 lines: comment, label, 4 instructions, rts
      expect(result.errorCount).toBe(0);
      expect(result.lines[1].parsed.label?.label).toBe("multiply_by_10");
    });
  ...
  ds.b 100`;
      ...
      expect(result.lines[1].parsed.mnemonic?.type).toBe("directive");
      expect(result.lines[2].parsed.mnemonic?.type).toBe("directive");
      expect(result.lines[3].parsed.mnemonic?.type).toBe("directive");
      expect(result.lines[4].parsed.mnemonic?.type).toBe("directive");
    });
  ...
  describe("edge cases", () => {
    ...
    it("should preserve line text exactly", () => {
      const source = `  move.w   d0,  d1
\t\tadd.l\td2,d3
label:  sub.w d4,d5`;
      const result = parseFile(source);
      expect(result.lines[0].text).toBe("  move.w   d0,  d1");
      expect(result.lines[1].text).toBe("\t\tadd.l\td2,d3");
      expect(result.lines[2].text).toBe("label:  sub.w d4,d5");
    });
